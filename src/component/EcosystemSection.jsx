    ...
    if (typeof window !== 'undefined') {
      import('gsap').then((gsap) => {
        ...
        masterTl.fromTo(titleRef.current,
          ...
          { opacity: 1, y: 0, scale: 1, rotationX: 0, duration: 1.2, ease: "elastic.out(1, 0.5)" },
          0.3
        );
        ...
        masterTl.fromTo(descWords,
          ...
          { opacity: 1, y: 0, rotationX: 0, duration: 0.8, stagger: 0.05, ease: "back.out(1.7)" },
          "-=0.8"
        );
        ...
        cards.forEach((card) => {
          ...
          card.addEventListener('mouseenter', () => {
            ...
            hoverTl
              .to(card, { scale: 1.15, rotationY: 5, z: 50, duration: 0.4, ease: "back.out(1.7)" })
              .to(icon, { rotation: 360, scale: 1.2, duration: 0.6, ease: "back.out(2)" }, 0)
              .to(text, { color: "#C570FB", textShadow: "0 0 20px rgba(197, 112, 251, 0.5)", duration: 0.3 }, 0);
          });
          card.addEventListener('mouseleave', () => {
            const leaveTl = gsapCore.timeline();
            leaveTl
              .to(card, { scale: 1, rotationY: 0, z: 0, duration: 0.4, ease: "power2.out" })
              .to(icon, { rotation: 0, scale: 1, duration: 0.6, ease: "back.out(1.7)" }, 0)
              .to(text, { color: "#f1f5f9", textShadow: "none", duration: 0.3 }, 0);
          });
        });
      });
    }
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto text-center relative z-10">
        ...
        <div ref={descriptionRef} className="text-gray-300 text-base sm:text-lg md:text-xl mx-auto mb-20">
            ...
            >
              <div className="bg-gradient-to-br from-[#1a1a2e]/80 via-[#16213e]/60 to-[#0f3460]/80 backdrop-blur-xl relative rounded-2xl p-8 shadow-2xl text-center flex items-center justify-center flex-col border border-purple-500/20 min-h-48 transition-all duration-500">
                ...
                <div className="card-icon bg-gradient-to-br from-[#5E2FF4] via-[#C570FB] to-[#FF3CAC] absolute -top-8 w-16 h-16 flex items-center justify-center rounded-full text-white shadow-2xl overflow-hidden border-4 border-white/20">
                  {card.logo ? (
                    <img src={card.logo} alt={`Logo ${idx + 1}`} className="w-8 h-8 object-contain filter brightness-110" />
                  ) : (
