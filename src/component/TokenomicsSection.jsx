...
import { getTokenomics, getAllMarketStats, getAllTokenTrackers } from "../api/admin-api";
import AOS from 'aos';
...
export default function TokenomicsTabs() {
  ...
  const initializeAnimations = () => {
    ...
    if (tabsRef.current) {
      ...
      gsap.fromTo(tabButtons,
        ...
        { opacity: 1, y: 0, scale: 1, duration: 0.6, stagger: 0.1, delay: 0.5, ease: "power2.out" }
      );
    }
  ...
  useEffect(() => {
    ...
    if (activeTab === "Token" && chartRef.current && tableRef.current && !loading) {
    ...
    if (activeTab === "Tracker" && trackerRef.current && !loading) {
      ...
      if (cards) {
        gsap.fromTo(cards,
          ...
          { opacity: 1, y: 0, rotationX: 0, duration: 0.7, stagger: 0.15, ease: "power2.out" }
        );
      }

      // Stats counter animation
      const statsElements = trackerRef.current?.querySelectorAll('.stat-number');
      if (statsElements) {
        statsElements.forEach(el => {
          gsap.from(el, {
            ...
            snap: { textContent: 1 },
            onUpdate: function() {
              el.textContent = Math.floor(this.targets()[0].textContent).toLocaleString();
            }
          });
        });
      }
 ...
 const renderTable = () => (
        ...
        <tbody className="bg-[#120540]/50">
          {tokenomicsData.map((item) => (
            ...
            >
              ...
              <td className="text-[0.9rem] text-[#AEA7D9]">{item.percentage}%</td>
              <td className="text-[0.9rem] text-[#AEA7D9]">{item.tokenQuantity.toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  const renderMarket = () => {
    if (loading) return <p className="text-center text-[#AEA7D9]">Loading market data...</p>;
    if (error) return <p className="text-red-400 text-center">{error}</p>;
    if (!marketStats) return null;

    return (
      <div ref={marketRef} className="bg-gradient-to-br from-[#120540]/60 via-[#1b0a2d]/60 to-[#433C73]/60 rounded-xl p-8 border border-[#727FA6]/30 break-words whitespace-normal">
        <div className="flex flex-col items-center justify-between mb-6 break-words whitespace-normal">
        <p className="text-lg font-semibold text-[#AEA7D9] mb-2 break-words whitespace-normal">Today's Market Price:</p>
        ...
        <p className={`text-lg font-semibold ${marketStats.change24h >= 0 ? 'text-green-400' : 'text-red-400'} break-words whitespace-normal`}>
          {marketStats.change24h >= 0 ? '+' : ''}{marketStats.change24h}% (24h)
        </p>
        </div>

        <div className="grid grid-cols-2 gap-6 mt-6">
          <div className="text-center break-words whitespace-normal">
            <p className="text-lg font-bold text-white break-words whitespace-normal">Market Cap</p>
            <p className="text-[#AEA7D9] break-words whitespace-normal">${marketStats.marketCap}</p>
          </div>
          <div className="text-center break-words whitespace-normal">
            <p className="text-lg font-bold text-white break-words whitespace-normal">Volume 24h</p>
            <p className="text-[#AEA7D9] break-words whitespace-normal">${marketStats.volume24h}</p>
          </div>
        </div>
      </div>
    );
  ...
  const renderTracker = () => {
    if (loading) return <p className="text-center text-[#AEA7D9]">Loading tracker data...</p>;
    if (error) return <p className="text-red-400 text-center">{error}</p>;
    if (!tokenTrackers) return null;

    return (
      <div ref={trackerRef} className="bg-gradient-to-br from-[#120540]/60 via-[#1b0a2d]/60 to-[#433C73]/60 rounded-xl p-8 border border-[#727FA6]/30 break-words whitespace-normal">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="p-6 rounded-xl border border-[#727FA6]/30 bg-[#433C73]/20 break-words whitespace-normal">
            <p className="text-xl text-[#AEA7D9] font-bold mb-1 break-words whitespace-normal">Total Holders</p>
            <p className="text-3xl font-bold text-white break-words whitespace-normal">{tokenTrackers.totalHolders.toLocaleString()}</p>
            <p className={`text-sm ${parseFloat(tokenTrackers.holdersChange) >= 0 ? 'text-green-400' : 'text-red-400'} break-words whitespace-normal`}>{tokenTrackers.holdersChange}% this week</p>
          </div>
          <div className="p-6 rounded-xl border border-[#727FA6]/30 bg-[#4A088C]/20 break-words whitespace-normal">
            <p className="text-xl text-[#AEA7D9] font-bold mb-1 break-words whitespace-normal">Total Transactions</p>
            <p className="text-3xl font-bold text-white break-words whitespace-normal">{tokenTrackers.totalTransactions.toLocaleString()}</p>
            <p className={`text-sm ${parseFloat(tokenTrackers.transactionsChange) >= 0 ? 'text-green-400' : 'text-red-400'} break-words whitespace-normal`}>{tokenTrackers.transactionsChange}% this week</p>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
          <div className="bg-[#120540]/50 p-4 rounded-lg text-center break-words whitespace-normal">
            <p className="text-[#AEA7D9] text-sm font-medium break-words whitespace-normal">Circulating Supply</p>
            <p className="text-white font-bold break-words whitespace-normal">{tokenTrackers.circulatingSupply.toLocaleString()}</p>
          </div>
          <div className="bg-[#120540]/50 p-4 rounded-lg text-center break-words whitespace-normal">
            <p className="text-[#AEA7D9] text-sm font-medium break-words whitespace-normal">Total Supply</p>
            <p className="text-white font-bold break-words whitespace-normal">{tokenTrackers.totalSupply.toLocaleString()}</p>
          </div>
          <div className="bg-[#120540]/50 p-4 rounded-lg text-center break-words whitespace-normal">
            <p className="text-[#AEA7D9] text-sm font-medium break-words whitespace-normal">Burned Tokens</p>
            <p className="text-white font-bold break-words whitespace-normal">{tokenTrackers.BurnedToken.toLocaleString()}</p>
          </div>
          <div className="bg-[#120540]/50 p-4 rounded-lg text-center break-words whitespace-normal">
            <p className="text-[#AEA7D9] text-sm font-medium break-words whitespace-normal">Max Supply</p>
            <p className="text-white font-bold break-words whitespace-normal">1234567890</p>
          </div>
        </div>
      </div>
    );
    ...
    <section  className="relative py-16 px-4 sm:px-6 md:px-12 bg-gradient-to-b from-[#433C73] via-[#1b0a2d] to-[#120540]">
      <div className="max-w-7xl mx-auto">
        <div ref={headerRef} className="text-center mb-10" data-aos="fade-down">
          ...
          <div ref={tabsRef} className="flex justify-center flex-wrap gap-4" data-aos="fade-up" data-aos-delay="200">
            {tabs.map((tab) => (
              <button
                key={tab}
      ...
      >
        <PieChart>
          ...
          />
              ...
              <div className="bg-gradient-to-br from-[#120540]/70 via-[#1b0a2d]/70 to-[#433C73]/70 rounded-xl p-6 border border-[#727FA6]/50" data-aos="zoom-in" data-aos-delay="700">
                <h3 className="text-xl font-bold text-center mb-4 text-[#AEA7D9]">Detailed Breakdown</h3>
                {loading ? <p className="text-center">Loading table...</p> : error ? <p className="text-red-400 text-center">{error}</p> : renderTable()}
              </div>
            </div>
          )}

          {activeTab === "Market" && (
            <div data-aos="fade-up" data-aos-delay="600">{renderMarket()}</div>
          )}

          {activeTab === "Tracker" && (
            <div data-aos="fade-up" data-aos-delay="600">{renderTracker()}</div>
          )}
        </div>
      </div>
    </section>
